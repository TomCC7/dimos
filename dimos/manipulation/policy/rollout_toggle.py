...
``PolicyNode``'s ``start_rollout`` / ``stop_rollout`` RPCs.
...
under `dimos/manipulation/policy/` because it imports `PolicyNode` —
...
from dimos.manipulation.policy.node import PolicyNode
...
class RolloutToggle(Module):
    """Toggle a `PolicyNode`'s rollout state on a configurable button edge."""
    ...
    def _on_buttons(self, msg: Buttons) -> None:
        ...
        try:
            if self.policy_node.is_rollout_active():
                self.policy_node.stop_rollout()
                logger.info("RolloutToggle: rollout stopped")
            else:
                self.policy_node.start_rollout()
