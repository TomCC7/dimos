...
directly with a fake `PolicyNode` so the tests stay hermetic — no LCM
...
class _FakePolicyNode:
    """Fake `PolicyNode` exposing the surface `RolloutToggle` touches.
...
def _make_toggle(button: str = "left_secondary", debounce: float = 0.5) -> Any:
    ...
    rt.policy_node = _FakePolicyNode()
    return rt
...
def test_rising_edge_starts_rollout() -> None:
    ...
    try:
        ...
        assert rt.policy_node.start_calls == 0
        rt._on_buttons(_make_buttons(left_secondary=True))
        assert rt.policy_node.start_calls == 1
        assert rt.policy_node.stop_calls == 0
        assert rt.policy_node.is_rollout_active() is True
    finally:
        rt.stop()
...
def test_rising_edge_when_active_stops_rollout() -> None:
    ...
    try:
        # Force active state.
        rt.policy_node._active = True

        rt._on_buttons(_make_buttons(left_secondary=False))
        rt._on_buttons(_make_buttons(left_secondary=True))
        assert rt.policy_node.stop_calls == 1
        assert rt.policy_node.start_calls == 0
        assert rt.policy_node.is_rollout_active() is False
    finally:
        rt.stop()
...
def test_debounce_window_suppresses_repeated_edges() -> None:
    ...
    try:
        ...
        assert rt.policy_node.start_calls + rt.policy_node.stop_calls == 1
    finally:
        rt.stop()
