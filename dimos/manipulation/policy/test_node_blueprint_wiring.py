...
"""End-to-end wiring: `PolicyNode` (with `TestPolicy`) → coordinator-compatible
joint command path.
...
when `task_type="servo"`) and feeds the `PolicyNode`'s published
...
def test_policy_publishes_joint_state_consumable_by_servo_task(policy_node):
    ...
    policy_node._on_joint_state(JointState(name=["arm/j1", "arm/j2"], position=[0.0, 0.0]))
    cmd = policy_node._tick_once()
