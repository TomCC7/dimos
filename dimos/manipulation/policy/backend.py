...
"""Backend protocol for `PolicyNode`.
...
class PolicyBackend(Protocol):
    ...
    Lifecycle:
        ...
        2. `initialize()` is called once on `PolicyNode.start()`. Heavy
        ...
        5. `close()` is called once on `PolicyNode.stop()`.
    """

    def initialize(self) -> None:
        """One-time backend setup. Called from `PolicyNode.start()`."""
    ...
    def close(self) -> None:
        """Release backend resources. Called from `PolicyNode.stop()`."""
