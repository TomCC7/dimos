...
The recommended path for offline training is to convert per-episode `.rrd`
files into a `LeRobotDataset` v2 directory using the in-tree converter — see
**[Datasets](/docs/usage/datasets.md)** for the CLI, gripper-binarization controls, and
output layout.
